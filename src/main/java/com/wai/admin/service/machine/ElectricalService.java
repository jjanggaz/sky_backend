...
public class ElectricalService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ElectricalService.class);
     ...
     * 전기 목록 조회 - 외부 API 호출
     */
    public Map<String, Object> getElectricalList(Map<String, Object> searchParams, String waiLang) {
        logger.debug("외부 인증 서버 전기 목록 조회 시도: server={}, params={}, waiLang={}", authServerBaseUrl, searchParams,
                waiLang);
        Map<String, Object> result = new HashMap<>();

        // 외부 인증 서버 URL 구성
        String electricalUrl = authServerBaseUrl + "/api/v1/equipment/equipment_catalog/search"
                + "?enable_dynamic_search=true"
                + "&include_hierarchy=true";
        logger.debug("전기 목록 조회 URL: {}", electricalUrl);

        // 요청 본문 구성 - 화면에서 받은 파라미터 사용
        Map<String, Object> requestMap = new HashMap<>();

        // 기본값 설정
        requestMap.put("top_root_level", searchParams.getOrDefault("top_root_level", "ELCTR"));
        ...
        logger.debug("전기 목록 조회 요청 본문: {}", requestBody);

        // HttpUtil을 사용하여 POST 요청 수행 (자동 토큰 추출)
        HttpUtil.HttpResult httpResult = HttpUtil.post(electricalUrl, "application/json", requestBody);
        ...
        if (httpResult.isSuccess()) {
            ...
            if (parsedResponse != null) {
                ...
                logger.warn("전기 목록 조회 응답 파싱 실패: {}", responseBody);
            }

            logger.debug("전기 목록 조회 성공");
            result.put("success", true);
            ...
            logger.warn("외부 인증 서버 전기 목록 조회 실패: {}", errorMessage);
        }
    ...
    /**
     * 전기 엑셀 템플릿 다운로드 - 외부 API 호출 (GET)
     */
    public Map<String, Object> downloadTemplateExcel(String equipmentType) {
        logger.debug("외부 인증 서버 전기 엑셀 템플릿 다운로드 시도: server={}, equipmentType={}", authServerBaseUrl, equipmentType);
        Map<String, Object> result = new HashMap<>();

        // 외부 인증 서버 URL 구성
        String downloadUrl = authServerBaseUrl + "/api/v1/equipment/equipment_catalog/migration/download/"
                + equipmentType;
        logger.debug("전기 엑셀 템플릿 다운로드 URL: {}", downloadUrl);
        ...
        if (httpResult.isSuccess()) {
            ...
            // 디버깅용: 파싱된 응답을 보기 좋게 로그 출력
            if (parsedResponse != null) {
                logger.debug("전기 엑셀 템플릿 다운로드 성공 응답: {}", JsonUtil.objectMapToJson(parsedResponse));
                result.put("success", true);
                result.put("data", parsedResponse);
                result.put("message", "전기 엑셀 템플릿 다운로드가 완료되었습니다.");
            } else {
                logger.error("전기 엑셀 템플릿 다운로드 응답 파싱 실패");
                result.put("success", false);
                result.put("message", "응답 파싱에 실패했습니다.");
            }
        } else {
            // 실패한 응답 처리
            logger.error("전기 엑셀 템플릿 다운로드 실패: statusCode={}, body={}", httpResult.getStatus(), httpResult.getBody());
            result.put("success", false);
            ...
            String errorMessage = httpResult.getExtractedErrorMessage();
            if (errorMessage == null || errorMessage.isEmpty()) {
                errorMessage = "전기 엑셀 템플릿 다운로드에 실패했습니다.";
            }
    ...
    public Map<String, Object> uploadModelExcel(String equipmentType, MultipartFile excelFile) {
        ...
        if (httpResult.isSuccess()) {
            ...
            // 디버깅용: 파싱된 응답을 보기 좋게 로그 출력
            if (parsedResponse != null) {
                logger.debug("전기 엑셀 파일 업로드 성공 응답: {}", JsonUtil.objectMapToJson(parsedResponse));
                result.put("success", true);
                result.put("data", parsedResponse);
                result.put("message", "전기 엑셀 파일 업로드가 완료되었습니다.");
            } else {
                logger.error("전기 엑셀 파일 업로드 응답 파싱 실패");
                result.put("success", false);
                result.put("message", "응답 파싱에 실패했습니다.");
            }
        } else {
            // 실패한 응답 처리
            logger.error("전기 엑셀 파일 업로드 실패: statusCode={}, body={}", httpResult.getStatus(), httpResult.getBody());
            result.put("success", false);
            ...
            String errorMessage = httpResult.getExtractedErrorMessage();
            if (errorMessage == null || errorMessage.isEmpty()) {
                errorMessage = "전기 엑셀 파일 업로드에 실패했습니다.";
            }
    ...
    public Map<String, Object> uploadModelZip(String machineName, MultipartFile allFile) {
        ...
        if (httpResult.isSuccess()) {
            ...
            // 디버깅용: 파싱된 응답을 보기 좋게 로그 출력
            if (parsedResponse != null) {
                logger.debug("전기 파일 업로드 성공 응답: {}", JsonUtil.objectMapToJson(parsedResponse));
                result.put("success", true);
                result.put("data", parsedResponse);
                result.put("message", "전기 파일 업로드가 완료되었습니다.");
            } else {
                logger.error("전기 파일 업로드 응답 파싱 실패");
                result.put("success", false);
                result.put("message", "응답 파싱에 실패했습니다.");
            }
        } else {
            // 실패한 응답 처리
            logger.error("전기 파일 업로드 실패: statusCode={}, body={}", httpResult.getStatus(), httpResult.getBody());
            result.put("success", false);
            ...
            String errorMessage = httpResult.getExtractedErrorMessage();
            if (errorMessage == null || errorMessage.isEmpty()) {
                errorMessage = "전기 파일 업로드에 실패했습니다.";
            }
    ...
    /**
     * 전기 삭제 - 외부 API 호출 (DELETE) 및 추가 처리
     */
    public Map<String, Object> deleteElectrical(String equipmentId, Map<String, Object> deleteParams) {
        logger.debug("전기 삭제 처리 시작: equipmentId={}", equipmentId);
        Map<String, Object> result = new HashMap<>();
        ...
        try {
            ...
            if ((Boolean) deleteResult.get("success")) {
                ...
                result.put("success", true);
                if (allFilesDeleted) {
                    result.put("message", "전기와 모든 관련 파일이 성공적으로 삭제되었습니다.");
                } else {
                    result.put("message", "전기는 삭제되었지만 일부 파일 삭제에 실패했습니다: " + errorMessages.toString());
                }
                result.put("data", deleteResult.get("data"));

                logger.debug("전기 삭제 완료: equipmentId={}, allFilesDeleted={}", equipmentId, allFilesDeleted);
            } else {
                // 전기 삭제 실패
                result = deleteResult;
                logger.error("전기 삭제 실패: equipmentId={}, message={}", equipmentId, deleteResult.get("message"));
            }

        } catch (Exception e) {
            logger.error("전기 삭제 처리 중 예외 발생: equipmentId={}, error={}", equipmentId, e.getMessage(), e);
            result.put("success", false);
            result.put("message", "전기 삭제 처리 중 오류가 발생했습니다: " + e.getMessage());
        }
    ...
    private Map<String, Object> deleteEquipmentFromAPI(String equipmentId) {
        ...
        if (httpResult.isSuccess()) {
            ...
            String responseBody = httpResult.getBody();
            logger.debug("전기 삭제 성공: statusCode={}, responseBody={}", httpResult.getStatus(), responseBody);

            result.put("success", true);
            result.put("message", "전기 삭제가 완료되었습니다.");
            ...
            // 실패한 응답 처리
            logger.error("전기 삭제 실패: statusCode={}, body={}", httpResult.getStatus(), httpResult.getBody());
            result.put("success", false);
    ...
    public Map<String, Object> getDetailCommon(String equipmentType) {
        logger.debug("전기 상세 업체 정보 조회 시작: equipmentType={}", equipmentType);
        Map<String, Object> result = new HashMap<>();
        ...
        try {
            ...
            logger.debug("전기 상세 업체 정보 조회 완료: equipmentType={}", equipmentType);

        } catch (Exception e) {
            logger.error("전기 상세 업체 정보 조회 중 예외 발생: equipmentType={}, error={}",
    ...
    public Map<String, Object> getDetailFiles(String equipmentId, Map<String, Object> fileParams) {
        ...
        try {
            ...
            logger.debug("전기 상세 파일 다운로드 완료: equipmentId={}, allFilesDownloaded={}", equipmentId, allFilesDownloaded);

        } catch (Exception e) {
            logger.error("전기 상세 파일 다운로드 중 예외 발생: equipmentId={}, error={}", equipmentId, e.getMessage(), e);
            result.put("success", false);
     ...
     * 전기 업데이트
     */
    public Map<String, Object> updateElectrical(String equipmentId, String updateParamsJson,
            MultipartFile dtdModelFile, MultipartFile thumbnailFile,
            MultipartFile revitModelFile, MultipartFile symbolFile) {
        logger.debug("전기 업데이트 처리 시작: equipmentId={}", equipmentId);

        try {
            Map<String, Object> updateResult = updateEquipmentFromAPI(equipmentId, updateParamsJson);

            if ((Boolean) updateResult.get("success")) {
                logger.debug("전기 업데이트 완료: equipmentId={}", equipmentId);
            } else {
                logger.error("전기 정보 업데이트 실패: equipmentId={}, message={}", equipmentId, updateResult.get("message"));
            }

            return updateResult;
        } catch (Exception e) {
            logger.error("전기 업데이트 처리 중 예외 발생: equipmentId={}, error={}", equipmentId, e.getMessage(), e);
            Map<String, Object> result = new HashMap<>();
            result.put("success", false);
            result.put("message", "전기 업데이트 처리 중 오류가 발생했습니다: " + e.getMessage());
            return result;
        }
    }
    ...
    private Map<String, Object> updateEquipmentFromAPI(String equipmentId, String updateParamsJson) {
        logger.debug("외부 인증 서버 전기 정보 업데이트 시도: server={}, equipmentId={}", authServerBaseUrl, equipmentId);
        Map<String, Object> result = new HashMap<>();
        ...
        try {
            ...
            if (httpResult.isSuccess()) {
                ...
                logger.debug("전기 정보 업데이트 성공: statusCode={}", httpResult.getStatus());

                result.put("success", true);
                result.put("message", "전기 정보 업데이트가 완료되었습니다.");
                result.put("data", parsedResponse);
            } else {
                // 실패한 응답 처리
                logger.error("전기 정보 업데이트 실패: statusCode={}, body={}", httpResult.getStatus(), httpResult.getBody());
                result.put("success", false);
                ...
                String errorMessage = httpResult.getExtractedErrorMessage();
                if (errorMessage == null || errorMessage.isEmpty()) {
                    errorMessage = "전기 정보 업데이트에 실패했습니다.";
                }
            ...
            logger.error("전기 정보 업데이트 처리 중 예외 발생: {}", e.getMessage(), e);
            result.put("success", false);
            result.put("message", "전기 정보 업데이트 처리 중 오류가 발생했습니다: " + e.getMessage());
        }
     ...
     * @return 업로드 결과
     */
    public Map<String, Object> createElectricalFormula(MultipartFile pythonFile, String formulaId) {
     ...
     * @return 삭제 결과
     */
    public Map<String, Object> deleteElectricalFormula(String formulaId) {
